...
const directContactEmail = 'engagement@gridanddot.com'
...
function buildMailtoHref({
  ...
  return `mailto:${directContactEmail}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`
}
...
async function submitViaWeb3Forms(payload: {
  ...
  const response = await fetch(web3FormsEndpoint, {
    ...
    body: JSON.stringify({
      ...
      subject: payload.projectType
        ? `Grid & Dot website enquiry - ${payload.projectType}`
        : 'Grid & Dot website enquiry',
      from_name: payload.name,
      name: payload.name,
      email: payload.email,
      message: payload.message || 'No additional context provided.',
  ...
  if (!response.ok || body?.success === false) {
    throw new Error(body?.message || `Web3Forms error ${response.status}`)
  }
}
  ...
  if (formStatus === 'server-error' || formStatus === 'error') {
    return (
      <p className="form-status-error">
        Sorry, there was an error submitting the form. Please try again or email us directly at{' '}
        <a href={`mailto:${directContactEmail}`}>{directContactEmail}</a>.
      </p>
    )
  }
